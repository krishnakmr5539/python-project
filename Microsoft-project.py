...
for case in case_list:
    ...
    for dir in dir_list:
        if "_rsi" in dir:
            ...
            # Rsi_final_dir = "/volume/CSdata/krikumar/Microsoft/" + case + "/" + RSI_DIR
            # files_list = os.listdir(RSI_DIR)
            # print(files_list)
            # files = [files for files in dir_list if os.path.isfile(files)] #list comprehension to collect only files except directory
            #get RSI from the files available in the current directory
            # RSI = list()
            # os.chdir(Rsi_final_dir)
            # for i in files_list:
            #     print("printing",i)
            #     RSI.append(i)
            #     obj1 = healthcheck(i)
